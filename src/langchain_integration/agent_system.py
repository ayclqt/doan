...
class ProductComparisonInput(BaseModel):
    ...
    comparison_aspects: List[str] = Field(
        default=["price", "specs", "features"], description="Aspects to compare"
    )
...
class PriceAnalysisInput(BaseModel):
    ...
    include_promotions: bool = Field(
        default=True, description="Include current promotions"
    )
...
def resolve_conversation_reference_tool(
    reference: str, conversation_history: List[dict]
) -> str:
    """
...
def compare_products_tool(
    product1: str, product2: str, comparison_aspects: List[str] = None
) -> str:
    """
    ...
    try:
        ...
        product1_info = (
            "\n".join([doc.page_content for doc in results1[:2]])
            if results1
            else "Không tìm thấy thông tin"
        )

        # Search for product2
        results2 = retriever.invoke(f"{product2} thông số cấu hình")
        product2_info = (
            "\n".join([doc.page_content for doc in results2[:2]])
            if results2
            else "Không tìm thấy thông tin"
        )
        ...
        if "price" in comparison_aspects:
            ...
            if web_searcher.is_available():
                ...
                price_info = (
                    "\n".join([r.body for r in price_results[:2]])
                    if price_results
                    else ""
                )
            else:
...
def analyze_price_trends_tool(
    product_name: str, include_promotions: bool = True
) -> str:
    """
...
def analyze_conversation_context_tool(
    conversation_history: List[dict], current_question: str
) -> str:
    """
    ...
    try:
        if not conversation_history:
            return (
                f"Phân tích câu hỏi: '{current_question}' - Không có ngữ cảnh trước đó."
            )
...
Hãy sử dụng tools một cách thông minh để trả lời chính xác và đầy đủ nhất."""
    ...
    def process_query(
        ...
        try:
            ...
            self.logger.info(
                f"Agent processed query with {len(tools_used)} tools in {processing_time:.2f}s"
            )
