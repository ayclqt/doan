...
class LLMSearchAgent:
    ...
    def decide_search_strategy(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Failed to parse LLM decision JSON, fallback: {should_search}")
    ...
    def generate_search_queries(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("Failed to parse query generation JSON, using fallback")
    ...
    def execute_intelligent_search(
        ...
        if decision.should_search and self.web_searcher and self.web_searcher.is_available():
            ...
            expected_info = getattr(decision, 'expected_information', ['thông tin sản phẩm'])
            queries = self.generate_search_queries(question, expected_info, conversation_history)
            ...
            self.logger.info(f"Executed intelligent search: {len(search_results)} results from {len(queries)} queries")
    ...
    def _fallback_decision(self, question: str, vector_results: List[Any]) -> SearchDecision:
        """Fallback rule-based decision when LLM fails."""
        # Simple rule-based logic
        should_search = (
            not vector_results or
            len(vector_results) < 2 or
            any(keyword in question.lower() for keyword in ["giá", "so sánh", "mới", "hiện tại"])
        )
    ...
    def _fallback_query_generation(self, question: str, conversation_history: Optional[List[dict]]) -> List[str]:
        ...
        if conversation_history:
            for msg in conversation_history[-2:]:
                ...
                products = [
                    ...
                    'realme', 'oneplus', 'huawei', 'nokia', 'asus', 'acer',
                    'dell', 'hp', 'lenovo', 'macbook', 'ipad'
                ]
        ...
        if product_names:
            for product in list(product_names)[:2]:  # Max 2 products
                ...
                references = ["điện thoại trên", "sản phẩm trên", "thiết bị trên", "máy trên"]
